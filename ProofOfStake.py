...
class ProofOfStake():
    ...
    def __init__(self):
        ...
        self.stakers = {} #A list of stakers (Their public keys)
        self.setGenesisNodeStake() #Sets the genisis node's stake to 1, so a node can actually forge the first block.

    def setGenesisNodeStake(self):
        #Reads the file given and sets the node's stake to 1.
    ...
    def update(self, publicKeyString, stake):
        #Responsible for updating the stake amount for the public key. If the public key is already a staker
        #Then it will add the amount given to their current total.
    ...
    def get(self, publicKeyString):
        #This just returns every public key that's a staker.
    ...
    def forger(self, lastBlockHash):
        #Responsible for nominating a forger.
